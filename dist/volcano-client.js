(function webpackUniversalModuleDefinition(root, factory) {
	if(typeof exports === 'object' && typeof module === 'object')
		...
		define("volcano", [], factory);
	else if(typeof exports === 'object')
		exports["volcano"] = factory();
	else
		root["volcano"] = factory();
...
var _Snapshot = __webpack_require__(53);
...
var _database = __webpack_require__(50);

var _database2 = _interopRequireDefault(_database);

function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { default: obj }; }

function _initializeApp(config) {
  var _this = this;

  var serverURL = config.serverURL,
      bucketName = config.bucketName;

  this.socket = (0, _socket2.default)(serverURL);
  return {
...
exports.default = {
  initializeApp: function initializeApp(config) {
    return new _initializeApp(config);
  }
};
...
var _Database = __webpack_require__(51);
...
var Database = function () {
  function Database(_ref2) {
    ...
    this.socket_on = function (eventType, callback) {
      if (!callback.name) throw new Error('Anonymous callback not allowed.');
      var ref = _this.nodes.join('/');
      var room = ref + '-' + eventType;
      var key = _this.socket.id + '-' + room;
      if (_this.socket_listeners[key]) {
        _this.socket_room_data[key].push(callback);
      } else {
        _this.socket_room_data[key] = [callback];
      }
      _this.socket.on(room, callback);
    };
    this.socket_off = function (eventType, callback) {
      if (!callback.name) throw new Error('Anonymous callback not allowed.');
    };

    // Bind APIs
    this.on = _on2.default.bind(this);
    this.once = _once2.default.bind(this);
    this.set = _set2.default.bind(this);
    this.update = _update2.default.bind(this);
    this.push = _push2.default.bind(this);
    this.remove = _remove2.default.bind(this);
  }
  ...
  _createClass(Database, [{
    ...
    value: function ref(_ref) {
      // TODO: validation
      this.nodes = _ref.split('/').filter(function (node) {
        return !!node;
      });
      return this;
    }
...
exports.default = function (eventType, callback, cancelCallback, context) {
  var _this = this;

  var isOnce = arguments.length > 4 && arguments[4] !== undefined ? arguments[4] : false;
  var onceCompleteCallback = arguments[5];

  console.log('---on---');
  context && callback.bind(context);
  var ref = this.nodes.join('/');
  ...
  var room = this.bucketName + '/' + ref + '-' + eventType;

  var isInitialized = false;
  var queueWhenIniting = [];
  var initingEvent = room + '-initing';
  var initedEvent = room + '-inited';
  var onEvent = '' + room;
  console.log(onEvent);

  var isOnceComplete = false;
  var invokeCallback = function invokeCallback(_ref) {
    ...
    if (isOnce) {
      ...
      if (!isOnceComplete) {
        isOnceComplete = true;
        onceCompleteCallback({ snapshot: snapshot, error: error });
  ...
  var initingCB = function initingCB(_ref2, tellServerComplete) {
    var snapshotRaw = _ref2.snapshotRaw,
        error = _ref2.error;

    console.log('---initing---');
    invokeCallback({ error: error, snapshot: new _models.Snapshot(snapshotRaw) });
    tellServerComplete();
  };
  var initedCB = function initedCB() {
    console.log('---inited---');
    isInitialized = true;
    queueWhenIniting.forEach(function (result) {
      invokeCallback(result);
    });
    _this.socket.off(initingEvent, initingCB);
    _this.socket.off(initedEvent, initedCB);
  };
  var onCB = function onCB(_ref3) {
    var snapshotRaw = _ref3.snapshotRaw,
        error = _ref3.error;

    console.log('---on value change---');
    var result = { error: error, snapshot: new _models.Snapshot(snapshotRaw) };
    if (!isInitialized) {
  ...
  return function (callback, context) {
    context && callback.bind(context);
    _this.socket.emit('volcano-off', { room: onEvent }, function () {
      _this.socket.off(onEvent, onCB);
...
exports.default = function (eventType, callback, cancelCallback, context) {
  ...
  return new Promise(function (resolve, reject) {
    var offHandler = void 0;
    offHandler = _this.on(eventType, callback, cancelCallback, context, true, function (_ref) {
      var snapshot = _ref.snapshot,
          error = _ref.error;
...
exports.default = function (value) {
  ...
  return new Promise(function (resolve, reject) {
    _this.socket.emit('volcano-set', params, function (error) {
      if (error) return reject(error);
      return resolve();
    });
  });
...
exports.default = function (value) {
  ...
  return new Promise(function (resolve, reject) {
    _this.socket.emit('volcano-update', params, function (error) {
      if (error) return reject(error);
      return resolve();
    });
  });
...
exports.default = function (value) {
  this.nodes.push((0, _bsonObjectid2.default)().toHexString());
  if (!value) return this;
  return this.set(value);
};

var _bsonObjectid = __webpack_require__(58);
...
function ObjectID(arg) {
  ...
  if (isBuffer(arg) || Array.isArray(arg) && arg.length === 12) {
    ...
    buf = buffer(arg);
  } else if (/number|undefined/.test(typeof arg === 'undefined' ? 'undefined' : _typeof(arg))) {
...
//# sourceMappingURL=volcano-client.js.map